...
public class RecruiterServiceTest {
    ...
    @Test
    public void testServiceSearchRecruiterWithNull() throws Exception {
        List<Recruiter> actual = getRecruiterList(5);
        when(recruiterRepository.findAll()).thenReturn(actual);
        List<Recruiter> expected = recruiterService.searchRecruiters(null);
        yakshaAssert(currentTest(),
                (asJsonString(expected).equals(asJsonString(actual))
                        ? "true"
                        : "false"),
    ...
    @Test
    public void testServiceSearchRecruiterWithSearchName() throws Exception {
        String searchKey = randomStringWithSize(2);
        List<Recruiter> actual = getRecruiterList(5);
        when(recruiterRepository.findByRecruiterCompanyName(searchKey)).thenReturn(actual);
        List<Recruiter> expected = recruiterService.searchRecruiters(searchKey);
        yakshaAssert(currentTest(),
                (asJsonString(expected).equals(asJsonString(actual))
                        ? "true"
                        : "false"),
